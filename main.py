    ...
    try:
        if m.chat.type == "channel":
            ...
            if not is_authorized:
                await m.reply_photo(
                    photo=photologo,
                    caption="**Mʏ Nᴀᴍᴇ [DRM Wɪᴢᴀʀᴅ 🦋](https://t.me/Tgballa)\n\nYᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀᴄᴄᴇꜱꜱ ᴛᴏ ᴜꜱᴇ ᴛʜɪꜱ ʙᴏᴛ\nCᴏɴᴛᴀᴄᴛ [RealToppers](https://t.me/Tgballa) ғᴏʀ ᴀᴄᴄᴇꜱꜱ**",
...
async def unauthorized_handler(client, message: Message):
    await message.reply(
        "<b>Mʏ Nᴀᴍᴇ [DRM Wɪᴢᴀʀᴅ 🦋](https://t.me/Tgballa)</b>\n\n"
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            # ── TOPIC EXTRACTION from [] or () in raw title ──────────
            raw_title = links[i][0]
            t_match = re.search(r'[\(\[]([^\)\]]+)[\)\]]', raw_title)
            if t_match:
                topic_name = t_match.group(1).strip()
                # Remove the topic bracket from the display name
                ...
                if "drive" in url:
                    ...
                    try:
                        ...
                        if os.path.exists(filename):
                            ...
                            # ── SAVE TO DB ────────────────────────────────────
                            if sent_msg:
                                await save_to_db(bot, channel_id, sent_msg.id, link0)
                            count += 1
                        else:
